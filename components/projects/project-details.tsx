        ...
        <Card>
          ...
          <CardContent className="space-y-4">
              ...
              <div>
                ...
                <p>
                  {project.start_date && project.start_date !== "" ?
                    (() => {
                      try {
                        return format(new Date(project.start_date), "MMM d, yyyy");
                      } catch (error) {
                        return "Invalid date";
                      }
                    })() : "Not set"}
              ...
              <div>
                ...
                <p>
                  {project.end_date && project.end_date !== "" ?
                    (() => {
                      try {
                        return format(new Date(project.end_date), "MMM d, yyyy");
                      } catch (error) {
                        return "Invalid date";
                      }
                    })() : "No deadline"}
            ...
            <div>
              <p className="text-sm font-medium">Progress</p>
              <p>
                {project.progress ? `${project.progress}%` : 'Not set'}
              </p>
            </div>
          </CardContent>
        </Card>
          ...
          <CardContent className="space-y-4">
            <div className="flex items-center gap-2">
              ...
              <div>
                ...
                <p>${project.estimated_budget?.toLocaleString()}</p>
              </div>
            </div>
            ...
            <div className="flex items-center gap-2">
              ...
              <div>
                ...
                <p>${project.actual_budget?.toLocaleString()}</p>
              </div>
            </div>
            ...
            <div>
              ...
              <div className="flex items-center gap-2 mt-1">
                <Progress
                  value={project.estimated_budget ? (project.actual_budget! / project.estimated_budget) * 100 : 0}
                  className="h-2 w-full"
                />
                <span className="text-xs text-muted-foreground w-10">
                  {project.estimated_budget ? Math.round((project.actual_budget! / project.estimated_budget) * 100) : 0}%
                </span>
              </div>
            </div>
          </CardContent>
